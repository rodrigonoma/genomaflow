...
export class DepthViewerComponent implements OnInit, OnDestroy {
  /** URL assinada do PNG depth map (V2-F3.1 heightmap). */
  @Input({ required: true }) depthUrl!: string;
  /** URL assinada da textura (foto frontal). */
  @Input({ required: true }) textureUrl!: string;
  /** F3.1 = 'heightmap'; F3.2 = 'gltf'. */
