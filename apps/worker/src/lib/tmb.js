...
// Duplicated from apps/api/src/services/aesthetic-tmb.js to avoid cross-package dependency.
// Both versions must stay in sync — Mifflin-St Jeor + activity factors + goal adjustments.
// If you update the formula, update both files and bump test coverage.
...
function computeAll(profile) {
  ...
  return { tmb: Math.round(tmb), calories, macros, primary_goal };
}
