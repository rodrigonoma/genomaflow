...
async function fixCard({
  ...
  // User pediu skip de PR: agente faz commit + push DIRETO em main.
  // CI deploy.yml dispara automaticamente. Sem human review intermediário.
  const commitMessage = `fix(trello-${card.idShort}): ${card.name}

Trello #${card.idShort}
Approved by @${memberUsername} via /fix aprovado${hint ? '\nHint: ' + hint : ''}

Auto-generated by GenomaFlow Trello QA Agent (attempt ${attempt}).
Tests passed (test:unit) locally before push.`;

  const result = await githubPr.commitAndPushToMain({
    repoRoot,
