...
description: F5 entregue 2026-05-11 — consent reforçado para regiões sensíveis (breast/glutes/abdomen), auto-crop Sonnet Vision + sharp pixelate, gate /analyses + /photos com erro CONSENT_REINFORCED_MISSING, purge worker job diário 04h BRT (retenção 1 ano LGPD).
type: project
---
...
| Componente | Path | Função |
|---|---|---|
| Purge job | `apps/worker/src/jobs/aesthetic-purge-sensitive.js` | Diário 07:00 UTC (04:00 BRT). Query: `is_sensitive=true AND deleted_at IS NULL AND created_at < NOW()-365d` BATCH 100. Soft delete (audit_trigger captura) + S3 deleteFile best-effort (falha → warn). Idempotência via `alreadyRanToday` (busca soft-delete same UTC day). `forceRun` flag pra ops/test. |
| Scheduler integration | `apps/worker/src/notifications/scheduler.js` | tick() dispara `shouldPurgeRun(now)` (gate UTC hour 7). Try/catch non-fatal — purge não quebra ticks de notification. |
